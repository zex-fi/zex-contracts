...
import {UserDeposit, MockERC20, MockERC721} from "../typechain-types";
...
describe("UserDeposit", function () {
    ...
    let toAcount: SignerWithAddress;

    beforeEach(async function () {
        [owner, transferrer, toAcount] = await ethers.getSigners();

        // Deploy UserDeposit contract
        const UserDeposit = await ethers.getContractFactory("UserDeposit");
        userDeposit = (await UserDeposit.deploy(await owner.getAddress(), await transferrer.getAddress(), await toAcount.getAddress())) as unknown as UserDeposit;
    ...
    describe("Initialization", function () {
        it("Should set the correct roles", async function () {
            ...
            expect(await userDeposit.hasRole(await userDeposit.OPERATOR_ROLE(), await transferrer.getAddress())).to.be.true;
        });

        it("Should revert if initialized with zero address", async function () {
            const UserDeposit = await ethers.getContractFactory("UserDeposit");
            await expect(UserDeposit.deploy(ZeroAddress, await transferrer.getAddress(), await toAcount.getAddress())).to.be.revertedWithCustomError(userDeposit, "ZeroAddress");
            await expect(UserDeposit.deploy(await owner.getAddress(), ZeroAddress, await toAcount.getAddress())).to.be.revertedWithCustomError(userDeposit, "ZeroAddress");
            await expect(UserDeposit.deploy(await owner.getAddress(), await transferrer.getAddress(), ZeroAddress)).to.be.revertedWithCustomError(userDeposit, "ZeroAddress");
        });
    });
    ...
    describe("ERC20 Transfers", function () {
        ...
        it("Should transfer ERC20 tokens", async function () {
            ...
            await expect(userDeposit.connect(transferrer).transferERC20(erc20Token.getAddress(), amount))
                .to.emit(userDeposit, "ERC20Transferred")
                .withArgs(erc20Token.getAddress(), await toAcount.getAddress(), amount);

            expect(await erc20Token.balanceOf(await toAcount.getAddress())).to.equal(amount);
        });

        it("Should revert if called by non-transferrer", async function () {
            const amount = ethers.parseEther("10");
            await expect(userDeposit.connect(toAcount).transferERC20(erc20Token.getAddress(), amount))
                .to.be.revertedWith("AccessControl: account " + (await toAcount.getAddress()).toLowerCase() + " is missing role " + await userDeposit.OPERATOR_ROLE());
        });

        it("Should revert if transferring zero amount", async function () {
            await expect(userDeposit.connect(transferrer).transferERC20(erc20Token.getAddress(), 0))
    ...
    describe("ERC721 Transfers", function () {
        ...
        it("Should transfer ERC721 tokens", async function () {
            await expect(userDeposit.connect(transferrer).transferERC721(erc721Token.getAddress(), tokenId))
                .to.emit(userDeposit, "ERC721Transferred")
                .withArgs(erc721Token.getAddress(), await toAcount.getAddress(), tokenId);

            expect(await erc721Token.ownerOf(tokenId)).to.equal(await toAcount.getAddress());
        });

        it("Should revert if called by non-transferrer", async function () {
            await expect(userDeposit.connect(toAcount).transferERC721(erc721Token.getAddress(), tokenId))
                .to.be.revertedWith("AccessControl: account " + (await toAcount.getAddress()).toLowerCase() + " is missing role " + await userDeposit.OPERATOR_ROLE());
        });
        ...
        it("Should revert if contract is not the token owner", async function () {
            ...
            await expect(userDeposit.connect(transferrer).transferERC721(erc721Token.getAddress(), newTokenId))
    ...
    describe("Native Token Transfers", function () {
        ...
        it("Should transfer native tokens", async function () {
            ...
            await expect(userDeposit.connect(transferrer).transferNativeToken(amount))
                .to.emit(userDeposit, "NativeTokenTransferred")
                .withArgs(await toAcount.getAddress(), amount);

            const userBalance = await ethers.provider.getBalance(await toAcount.getAddress());
            expect(userBalance).to.be.gte(amount); // Check that the user's balance increased
        });

        it("Should revert if called by non-transferrer", async function () {
            const amount = ethers.parseEther("0.5");
            await expect(userDeposit.connect(toAcount).transferNativeToken(amount))
                .to.be.revertedWith("AccessControl: account " + (await toAcount.getAddress()).toLowerCase() + " is missing role " + await userDeposit.OPERATOR_ROLE());
        });


        it("Should revert if transferring zero amount", async function () {
            await expect(userDeposit.connect(transferrer).transferNativeToken(0))
                .to.be.revertedWithCustomError(userDeposit, "InvalidAmount");
        });
